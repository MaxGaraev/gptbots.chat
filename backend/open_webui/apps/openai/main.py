...
async def speech(request: Request, user=Depends(get_verified_user)):
    ...
    try:
        ...
        if "openrouter.ai" in app.state.config.OPENAI_API_BASE_URLS[idx]:
            headers["HTTP-Referer"] = "https://gptbots.chat/"
            headers["X-Title"] = "GPTBOTS.CHAT"
        ...
        try:
            ...
            log.exception(e)
            error_detail = "GPTBOTS.CHAT: Server Connection Error"
            if r is not None:
                try:
                    res = r.json()
                    if "error" in res:
    ...
    if url_idx is None:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            try:
                ...
                raise HTTPException(
                    status_code=500, detail="GPTBOTS.CHAT: Server Connection Error"
                )
            except Exception as e:

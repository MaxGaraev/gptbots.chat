...
class SPAStaticFiles(StaticFiles):
    # Пути, которые должны возвращать 200
    VALID_PATHS = {'/', '/auth'}
    ...
    async def get_response(self, path: str, scope):
        try:
            ...
            if ex.status_code == 404:
                response = await super().get_response("index.html", scope)
                response.status_code = 404  # Для всех остальных путей возвращаем 404
                return response
            raise ex
...
async def get_manifest_json():
    return {
        ...
        "description": "GPTBOTS.CHAT is an open, extensible, user-friendly interface for AI that adapts to your workflow.",
        "start_url": "/",
        "display": "standalone",
